...
if (!isset($_SESSION['loggedin']) OR $_SESSION['loggedin'] !==true) {
  ...
  $stmt = $db->prepare('SELECT * FROM ResultMBI WHERE 1');        
  $stmt->execute(array());      
  $row = $stmt->fetch(PDO::FETCH_ASSOC);

  // Recupérer les données de la table ResultWB
  $stmt = $db->prepare('SELECT * FROM ResultWB WHERE 1');        
  $stmt->execute(array());      
...
<body>
		...
		<div class="content-div" id="content-div">
			<div class="nav-vertical-div">
				<ul>
					...
					<li><a class="active" href="resultats.php">Resultats</a></li>
				</ul>
			</div>
			<div class="result-div" id="quiz-results">
				<div class="quiz-div"> 
					<h1 style='color: #009999;'>Resultats</h1>
					<h2 style='color:#FF5700;margin-top: 25px;'>Veuillez trouver ici vos résultats :</h2>
				</div>
    ...
    <script>
       ...
       var sepColor = function( pSEP ){ 
        ...
        var weekCount = 8;            
        var weeks = new Array("S1");                   // tableau poue les labels des semaines initialisé
        
        for(var idx = 1;  idx < weekCount; idx++ ){    // Nombre de semaine par defaut pour l'affichage des données
        	weeks.push("S"+(idx+1));                   // Labels seamines
        }
